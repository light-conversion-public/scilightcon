...
def load_THORLABS_filter_transmissions(
    ...
    data_file_name = f"{suffix}_THORLABS_{filter}.csv"
...
def load_hamamatsu_pmt_qe_data(pmt: Literal) -> Tuple[np.ndarray, list]:
    """
    Load Hamamatsu PMT QE data.

    Examples:
        >>> from scilightcon.datasets import load_hamamatsu_pmt_qe_data.
        >>> data, header = load_atmospheric_data('H7422-40')
        >>> np.shape(data)
        (2251, 2)
        >>> header
        ['Wavelength  (nm)', 'Transmission (%)']
    ...
    Args:
        filter (str): `H7422-40', `H7422-50`, `H10721-01`, `H10721-110`, `H10721-210`

    Returns:
        data (Ndarray): A 2D array of data with headers excluded. Shape (n_samples, n_columns)
        header (List): Column names or empty strings. Shape (n_columns)
    """
    data_file_name = f'QE_Hamamatsu_{pmt}.csv'

    try:
        data, header = load_csv_data(
            data_file_name=data_file_name,
            data_module=DATA_MODULE_HAMAMATSU
        )
    except FileNotFoundError:
        raise ValueError
